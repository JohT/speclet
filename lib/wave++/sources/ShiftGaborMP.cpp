		    ...
		    )
{
  ...
  real_DWT coef=0.0, error=0.0;
  RealGabor Gtemp;
  
  // Initialize Rf and f_approx
  f_approx.Set(0, dim-1);
  Rf = f;    
  real_DWT *Gptr, *fptr, *Rfptr;
		     ...
		     )
{
  ...
  real_DWT ddv;        // ddv will hold dv[j]
  integer lmshift = Part.lmu - c, rmshift = Part.rmu - c; // leftmost and rightmost
  // shifts with respect to the central position c
  real_DWT codv, sidv, covshift, sivshift; // codv will hold cos( ddv )
                                       // covshift will hold cos( v * shift )
  Interval CO, SI; // CO[t] will hold cos( t * v )

  integer  beg, end;
  Interval P, Q;
  real_DWT a, b, phase, Pnorm2, Qnorm2, PprodQ, a1, b1, product;
  real_DWT atemp, btemp, Pnorm2temp, Qnorm2temp, PprodQtemp;
  real_DWT *Pptr, *Qptr, *fptr, *COptr, *SIptr; 
  real_DWT covlms, sivlms, covdu, sivdu, codvlms, sidvlms, codvdu, sidvdu, temp;
  ...
  for( integer j = Part.s.beg; j <= Part.s.end; j++ )
    {
      ...
      *Pptr++ = g((real_DWT)(beg - c) / Part.s.origin[j]);
      *COptr++ = cos( beg * ddv );    *SIptr++ = sin( beg * ddv );
      for(t = beg + 1; t <= end; t++) 
	{
	  *Pptr++ = g((real_DWT)(t - c)/Part.s.origin[j]);
      ...
      for(integer shift = lmshift; shift <= rmshift; shift += ddu)
	{
      ...
      for(real_DWT v=ddv; v < Part.rmv; v += ddv)
	{ 
