...
#define fftbitrev(out,in,q)  bitrevd(out,in,q,sizeof(complex_DWT))
	  ...
	  int   size)		/* Number of bytes in an `in[]' element.  */
{
  ...
  for(n=0; n<(1<<q); n++)
    {
      u = br(n,q);
      memcpy( (char*)out+n*size, (char*)in+u*size, size );
    }
  return;
}
...
{
  int b, j, k, N, N1, M;
  complex_DWT  *fptr1, *fptr2, tmp;
  const complex_DWT *Wptr;
	   ...
	   int M)		/* for n=0,1,2,...,|M|-1. */
{
  ...
  M = abs(M);
  W = (complex_DWT *)malloc(M*sizeof(complex_DWT));

  theta = 0.0;
  for(n=0; n<M; n++)
    {
      W[n].Re = (real_DWT)cos(theta);
      W[n].Im = (real_DWT)sin(theta);
      theta += factor;
    }
  return(W);
}
      ...
      int q)			/* point DFT of the input `f[]' */
{
  ...
  /* Allocate an output std::vector of same length, `N', as `f[]': */
  fhat = (complex_DWT *)calloc(N, sizeof(complex_DWT));
  ...
  /* Generate the complex_DWT exponential std::vector `Omega_{N/2}': */
  if(inverse)
