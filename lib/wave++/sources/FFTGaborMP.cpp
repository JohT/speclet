...
// RunFFTGaborMP
static real_DWT coef, product, a, b, a1, b1, v, phase, Pnorm2, Qnorm2, PprodQ;
...
void proces1(const integer &N, const real_DWT *fptr) 
{
  ...
  real_DWT temp;
  real_DWT *Xptr = X.origin, *Yptr = Y.origin;
  for(integer t=0; t < N; t++)
    {
      temp = g((real_DWT)(t-u)/(real_DWT)s);
      *Xptr++ = (*fptr++) * temp;
      *Yptr++ = temp * temp;
    }

  complex_DWT *E1, *E2, *O1, *O2;
  E1 = (complex_DWT *) calloc(N, sizeof(complex_DWT));
  assert(E1);
  E2 = (complex_DWT *) calloc(N, sizeof(complex_DWT));
  assert(E2);
...
void proces2(const integer &N, const integer &stop, const real_DWT *fptr)  
{
  ...
  for(integer t=0; t <= stop; t++)
    {
      temp = g((real_DWT)(t-4*s)/(real_DWT)s);
      *Xptr++ = (*fptr++) * temp;
      *Yptr++ = temp * temp;
    }
  ...
  real_DWT c = Y.origin[0];
  real_DWT alpha = 2 * M_PI * d / N ;
  real_DWT cosal, cos2al, sinal, sin2al;
  cosal = cos2al = 1;
  sinal = sin2al = 0;
  real_DWT co = cos(alpha), si = sin(alpha);
  real_DWT x, y, uu, vv;
  for(integer k = 0; k < (N>>1); k++)
    {
