...
class QMF {         // base class : aperiodic quadratic mirror filter
 public:
  ...
  QMF(const real_DWT *f, integer alpha, integer omega);  // construct QMF
  QMF(const QMF &);                                  // copy constructor
  virtual ~QMF();                                    // destructor
  virtual void Set(const real_DWT *f, integer alpha, integer omega);  
  integer beg;       // least index of coef array
  integer end;       // final index of coef array
  real_DWT center;       // center of energy of the filter
  real_DWT dev;          // deviation
  real_DWT *coef;        // array of coefficients : from coef[beg] to coef[end]
};

class GPQMF : public QMF { // derived class : periodized qmf
 public:                   // All even length periodizations 
  GPQMF();
  GPQMF(const real_DWT *f, integer alpha, integer omega);
  GPQMF(const GPQMF &);
  virtual ~GPQMF();
  virtual void Set(const real_DWT *f, integer alpha, integer omega);
  void periodize(real_DWT *fq, integer q);                    // utilty used by Set

  real_DWT **pcoef;                   // periodized coefficient array
...
class PQMF : public QMF { // derived class : periodized quadratic mirror filter
 public:                  // periodizations of length a power of 2
  PQMF();
  PQMF(const real_DWT *f, integer alpha, integer omega);
  PQMF(const PQMF &);
  virtual ~PQMF();
  virtual void Set(const real_DWT *f, integer alpha, integer omega);
  void periodize(real_DWT *fq, integer q);                    // utilty used by Set
