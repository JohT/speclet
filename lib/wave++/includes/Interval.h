...
public: 
  ...
  Interval(const integer &alpha, const integer &omega, const real_DWT *data = 0);
       // construct interval from data. data must be omega-alpha+1 long,
       // or else by default data=0 and all elts set to 0

  Interval(const integer &p, const real_DWT *data=0);       
  ...
  real_DWT &operator[](const integer &i);       // overload [] with  bound checking

  const real_DWT &operator[](integer i) const;  // overload [] for const objects

  const Interval &operator+=(const Interval &I);  // overload +=another interv.

  const Interval &operator+=(const real_DWT &c);      // overload +=scalar
  
  Interval operator+(const Interval &I) const;    // overload +another interval

  Interval operator+(const real_DWT &c) const;        // overload +scalar 

  const Interval &operator*=(const real_DWT &c);             // overload *=scalar
  
  Interval operator*(const real_DWT &c) const;        // overload *scalar
 
  void Set(integer alpha, integer omega, const real_DWT *data = 0);  
       // utility which  can be used for redefining an existing interval

  /////////////////////////////////////////////////////////////////////////////
  real_DWT* origin;                 // pointer to begining of data
...
real_DWT L2norm(const Interval &I);       // return L2 norm of I

real_DWT Average(const Interval &I);      // return average of all elements of I

real_DWT StDev(const Interval &I);        // return standard deviation of I
