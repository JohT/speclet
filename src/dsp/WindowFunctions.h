...
//------------------------------------------------------------------------------------//
class WindowFunction abstract {
public:
	WindowFunction(long resolution);
	~WindowFunction(void);
...
protected:
	virtual double calculateFactor(int index) = 0;	//will be redefined by the derived windowing function

	long	mResolution; //also known as N or FFT-size or length of signal piece array
	double* windowFunctionFactors;

private:
	WindowFunction() {}						//explicit no default contructor;
	WindowFunction(const WindowFunction &) {}	//explicit no copy contructor;
};
