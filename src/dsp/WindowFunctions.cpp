...
WindowFunction::~WindowFunction(void) {
	if (windowFunctionFactors) free(windowFunctionFactors);
}

double WindowFunction::getFactor(int index) {
	assert(index >= 0);
	assert(index < mResolution);

	return getWindow()[index];
}

double* WindowFunction::getWindow() {
	if (!windowFunctionFactors) {
...
WindowFunction* WindowFunctionsFactory::createWindowFunction(int windowFunctionNr, long resolution) {
	...
	case SpectronParameters::WINDOWING_BARTLETT	:{ result = (WindowFunction*)(new WindowBartlett(resolution));	break;}
	case SpectronParameters::WINDOWING_BLACKMAN	:{ result = (WindowFunction*)(new WindowBlackman(resolution));	break;}
	case SpectronParameters::WINDOWING_BLACKMAN_HARRIS:{ result = (WindowFunction*)(new WindowBlackmanHarris(resolution));break;}
	case SpectronParameters::WINDOWING_HAMMING	:{ result = (WindowFunction*)(new WindowHamming(resolution));		break;}
	case SpectronParameters::WINDOWING_HANNING	:{ result = (WindowFunction*)(new WindowHanning(resolution));		break;}
	case SpectronParameters::WINDOWING_PARZEN		:{ result = (WindowFunction*)(new WindowParzen(resolution));		break;}
	case SpectronParameters::WINDOWING_SQUARE		:{ result = (WindowFunction*)(new WindowSquare(resolution));		break;}
	case SpectronParameters::WINDOWING_WELCH		:{ result = (WindowFunction*)(new WindowWelch(resolution));		break;}
	default : {bool windowing_function_unknown = false; assert(windowing_function_unknown);}	
	}
...
WindowFunction* WindowFunctionsFactory::readWindowFunctionFromMap(int windowFunctionNr, long resolution) {
	...
	if (windowingFunctionsIterator == windowingFunctions.end()) return NULL;
	...
	if (resolutionsIterator == resolutions.end()) return NULL;
...
void WindowFunctionsFactory::deleteWindowFunctionsMap() {
	...
	TWindowFunctionsMapIterator windowingFunctionsIterator = windowFunctions.begin();
